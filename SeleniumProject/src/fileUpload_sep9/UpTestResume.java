...
public class UpTestResume {
    ...
    public static void uploadResumeInNaukri() {
        ...
        try {
            ...
            oAction.moveToElement(oBrowser.findElement(By.xpath("//button[text()='Register now']"))).perform();
            Thread.sleep(2000);
            oBrowser.findElement(By.xpath("//button[text()='Upload Resume']")).click();
            Thread.sleep(8000);

            //6. Handle the Upload window component using autoIT
            Runtime.getRuntime().exec("C:\\Users\\Admin\\OneDrive\\Documents\\UploadCV.exe");
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            oBrowser = null;
